...
todos:
  - id: due-status-logging
    content: "src/lib/habits: get-intention-due-status, get-due-intention-checkins, log-intention-checkin + Goal type uitbreiden"
    status: completed
  - id: ai-prompts
    content: src/lib/ai/generate-intention-checkin.ts + cache via dashboard_reflection_cache
    status: completed
  - id: checkin-ui
...
| Onderdeel | Status |
|-----------|--------|
| CRUD intenties op `/vandaag` | Werkt via `GoalsSection.tsx` + `src/lib/habits/*` |
