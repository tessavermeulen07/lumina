...
todos:
  - id: server-actions
    content: Maak src/lib/entries/ met saveEntry en deleteEntry server actions + NL-foutmeldingen
    status: pending
  - id: writing-area
...
Na registratie en onboarding landt de gebruiker op [`/schrijf?prompt=first_entry`](src/app/(app)/schrijf/page.tsx). [`WritingArea`](src/components/features/journal/WritingArea.tsx) houdt tekst alleen in lokale React state bij; er is geen persistence.
...
RLS staat INSERT/UPDATE/DELETE toe voor eigen entries (`auth.uid() = user_id`). Types bestaan in [`src/types/database.ts`](src/types/database.ts).
...
- Auth-check server-side via [`src/lib/supabase/server.ts`](src/lib/supabase/server.ts)
...
| Actie | Bestand |
|-------|---------|
| Nieuw | `src/lib/entries/save-entry.ts` — server action + validatie |
| Nieuw | `src/lib/entries/delete-entry.ts` — server action voor verwijderen |
| Nieuw | `src/lib/entries/errors.ts` — Nederlandse foutteksten |
| Wijzig | [`src/components/features/journal/WritingArea.tsx`](src/components/features/journal/WritingArea.tsx) — save-state, debounce, feedback |
| Wijzig | [`src/components/features/journal/WritingToolbar.tsx`](src/components/features/journal/WritingToolbar.tsx) — opslaan-knop in main panel |
| Wijzig | [`src/app/(app)/schrijf/page.tsx`](src/app/(app)/schrijf/page.tsx) — optioneel `promptType` doorgeven (voor toekomstige first-entry UX) |
...
In het **main panel** van [`WritingToolbar`](src/components/features/journal/WritingToolbar.tsx), naast bestaande iconen:
...
[`src/app/(app)/schrijf/page.tsx`](src/app/(app)/schrijf/page.tsx) blijft dun. Optioneel `promptType` doorgeven aan `WritingArea` als prop voor latere first-entry-specifieke feedback (bijv. "Je eerste entry is opgeslagen") — **niet verplicht voor MVP**.

## Buiten scope (opvolgstappen)

Deze items zijn **niet** nodig om de eerste entry op te slaan, maar wel logische vervolgstappen:

- [`src/app/(app)/entries/page.tsx`](src/app/(app)/entries/page.tsx) — entries ophalen en tonen i.p.v. placeholder
- [`src/lib/mock/dashboard.ts`](src/lib/mock/dashboard.ts) — vervangen door echte entry-count query
...
- [x] Plan opgeslagen in `docs/plans/entry-opslaan-database.md`
