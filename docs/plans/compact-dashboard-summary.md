...
- **Nu:** `getDashboardOverview()` in `src/lib/mock/dashboard.ts`
- **Later:** `src/lib/data/getDashboardOverview.ts` haalt entries uit Supabase en berekent streak, woorden en weekstatus
...
In [`DailyJournalSection.tsx`](src/components/features/dashboard/DailyJournalSection.tsx):
...
- `promptsPlaceholderMessage` in `src/lib/mock/dashboard.ts` opruimen indien ongebruikt
