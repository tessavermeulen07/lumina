...
todos:
  ...
  - id: verify-db
    ...
    status: completed
  - id: typescript-types
    content: Voeg QuestionCategory, QuestionFramework, Question, AiUseCase en Database.public.Tables.questions toe in src/types/database.ts (+ src/lib/ai/question-context.ts)
    status: completed
  - id: ai-use-case-mapping
    content: Maak src/lib/ai/question-context.ts met getQuestionsForUseCase() en toolbar-action mapping (vervolgstap na migratie)
    status: completed
...
Uitbreiden van [`src/types/database.ts`](src/types/database.ts) met vaste unions (nu de taxonomie bekend is):
...
**Ja — deze vragen zijn bedoeld om door de AI-agent te worden opgeput**, zowel op het dashboard als **tijdens het schrijven van een entry** op [`/schrijf`](src/app/(app)/schrijf/page.tsx). De database-categorieën sluiten niet 1-op-1 aan op UI-labels; de AI koppelt ze via een **use-case mapping** in applicatiecode.
...
|----------------|----------------------|--------------------------|---------------------|
| **Dagelijkse reflectie** | `emotieregulatie`, `patronen`, `stress_angst` | Emotie als vorm/kleur; energiek vs. leeggezogen; gedachte die je bezighoudt | [`DailyJournalSection`](src/components/features/dashboard/DailyJournalSection.tsx), WritingToolbar "Vraag" |
| **Patronen herkennen** | `patronen` (primair), `stress_angst` (triggers) | Ongeschreven regel; vermijdingsgedrag; helikoptervlucht | [`AskLuminaSection`](src/components/features/dashboard/AskLuminaSection.tsx), WritingToolbar "Eerdere gedragspatronen", [`ai_insights`](supabase/migrations/20250615000000_initial_schema.sql) |
| **Intenties** | `intenties` | 5-min actie; drempel verlagen; weekdoel-bewustzijn | [`GoalsSection`](src/components/features/dashboard/GoalsSection.tsx), `habits_and_intentions`, `habit_logs.ai_checkin_prompt` |
...
Op [`/schrijf`](src/app/(app)/schrijf/page.tsx) slaat [`WritingArea`](src/components/features/journal/WritingArea.tsx) de entry al op via `saveEntry` (auto-save + handmatig). De [`WritingToolbar`](src/components/features/journal/WritingToolbar.tsx) heeft 8 AI-knoppen die nu **mock/no-op** zijn — hier komt de `questions`-tabel het sterkst tot zijn recht: de AI leest de **huidige entry-inhoud** en put **gerichte vervolgvragen** uit de catalogus.
...
Nieuw bestand [`src/lib/ai/question-context.ts`](src/lib/ai/question-context.ts):
...
Server action [`src/lib/ai/respond-to-entry.ts`](src/lib/ai/respond-to-entry.ts) (vervolgstap):
...
- [`AskLuminaSection`](src/components/features/dashboard/AskLuminaSection.tsx) blijft `sampleQuestions` gebruiken tot AI-integratie
- [`WritingToolbar`](src/components/features/journal/WritingToolbar.tsx) AI-knoppen hebben nog geen `onClick` → `respondToEntry`
...
| Nieuw (vervolgstap) | `src/lib/ai/question-context.ts` — use-case + toolbar-action mapping |
| Nieuw (vervolgstap) | `src/lib/ai/respond-to-entry.ts` — server action voor AI-reactie op entry |
| Wijzig (vervolgstap) | [`WritingArea`](src/components/features/journal/WritingArea.tsx) + [`WritingToolbar`](src/components/features/journal/WritingToolbar.tsx) — AI-reactiepaneel + onClick-handlers |
