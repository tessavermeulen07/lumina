...
todos:
  - id: extract-marketing-home
    content: Extraheer MarketingHome uit src/app/(marketing)/page.tsx
    status: pending
  - id: create-input
    content: Maak src/components/ui/Input.tsx met Lumina field-styling
    status: pending
  ...
  - id: create-register-route
    content: Maak src/app/(marketing)/registreren/page.tsx
    status: pending
  - id: wire-ctas
...
Voor jouw ontwerp (donkere waas `bg-slate-900/50 backdrop-blur-sm` over de landing) volstaat **optie 1**: op [`src/app/(marketing)/registreren/page.tsx`](src/app/(marketing)/registreren/page.tsx) renderen we dezelfde landing-inhoud **én** een `fixed inset-0` overlay met de kaart erbovenop. De bezoeker ziet Header, Hero en features gedimd en wazig achter de modal.
...
- **Modal-structuur**: volg [`src/components/features/dashboard/AddGoalModal.tsx`](src/components/features/dashboard/AddGoalModal.tsx) — `fixed inset-0 z-50`, `role="dialog"`, Escape-toets, backdrop-klik sluit.
- **Input-styling**: hergebruik `fieldClassName` uit AddGoalModal (`rounded-xl border border-lumina-500/25 bg-surface ...`).
- **Button**: [`src/components/ui/Button.tsx`](src/components/ui/Button.tsx) — brede submit-knop via `className="w-full"`.
- **Logo**: [`src/components/ui/Logo.tsx`](src/components/ui/Logo.tsx) — cirkel-logo (L met sterretjes), subtiel met `opacity-70` en kleiner (`h-6 w-6`).

---

## Nieuwe en gewijzigde bestanden

### 1. `src/components/ui/Input.tsx` (nieuw)

Herbruikbaar label + input, conform projectstructuur (`src/components/ui/`). Props: `id`, `label`, `type`, `placeholder`, optioneel `autoComplete`.

### 2. `src/components/features/marketing/MarketingHome.tsx` (nieuw)

Extraheer de landing-compositie uit [`src/app/(marketing)/page.tsx`](src/app/(marketing)/page.tsx) (Header, main-secties) zodat `/` en `/registreren` dezelfde achtergrond delen zonder duplicatie.

### 3. `src/components/features/marketing/RegisterCard.tsx` (nieuw, `"use client"`)

Gecentreerde kaart met:

**Bovenkant**
- Subtiel [`Logo`](src/components/ui/Logo.tsx), gecentreerd
...
### 4. `src/components/features/marketing/RegisterOverlay.tsx` (nieuw, `"use client"`)
...
### 5. `src/app/(marketing)/registreren/page.tsx` (nieuw)
...
Wijzig in [`src/components/features/marketing/Hero.tsx`](src/components/features/marketing/Hero.tsx) en [`src/components/features/marketing/Header.tsx`](src/components/features/marketing/Header.tsx):
