...
todos:
  ...
  - id: f1-ai-prompt
    ...
    status: completed
  - id: f1-enforce
    content: "src/app/(app)/layout.tsx: redirect naar /onboarding als niet afgerond"
    status: completed
  - id: f1-verify
...
- Wizard: [`src/components/features/onboarding/OnboardingWizard.tsx`](src/components/features/onboarding/OnboardingWizard.tsx) — roept `completeOnboarding(coachStyle)` aan
- Opslag: [`src/lib/profile/complete-onboarding.ts`](src/lib/profile/complete-onboarding.ts) — 1 veld
- AI: [`src/lib/ai/agent-prompt.ts`](src/lib/ai/agent-prompt.ts), [`src/lib/ai/agent.ts`](src/lib/ai/agent.ts)
- Aanroepers: [`src/lib/ai/respond-to-entry.ts`](src/lib/ai/respond-to-entry.ts), [`src/lib/ai/ask-lumina.ts`](src/lib/ai/ask-lumina.ts)
- Profiel: [`src/types/database.ts`](src/types/database.ts) `Profile` — mist onboarding-velden

**Twee onboarding-paden** (beide moeten blijven werken):

1. Registratie-overlay: [`RegisterOverlay.tsx`](src/components/features/marketing/RegisterOverlay.tsx) → compact wizard → `/schrijf`
2. Standalone route: [`src/app/(onboarding)/onboarding/page.tsx`](src/app/(onboarding)/onboarding/page.tsx)
...
**Types bijwerken** in [`src/types/database.ts`](src/types/database.ts):
...
Import onboarding-types uit [`src/types/onboarding.ts`](src/types/onboarding.ts).
...
**Wijzig** [`src/lib/profile/complete-onboarding.ts`](src/lib/profile/complete-onboarding.ts):
...
**Wijzig** [`src/components/features/onboarding/OnboardingWizard.tsx`](src/components/features/onboarding/OnboardingWizard.tsx):
...
**Nieuw** `src/lib/profile/onboarding-context.ts`:
...
`buildOnboardingPromptContext` mapt ids naar labels via bestaande constanten in [`src/lib/constants/onboarding.ts`](src/lib/constants/onboarding.ts):
...
**Wijzig** [`src/lib/ai/agent-prompt.ts`](src/lib/ai/agent-prompt.ts):
...
**Wijzig** [`src/lib/ai/agent.ts`](src/lib/ai/agent.ts):
...
- [`src/lib/ai/respond-to-entry.ts`](src/lib/ai/respond-to-entry.ts) — heeft `getProfile()` al
- [`src/lib/ai/ask-lumina.ts`](src/lib/ai/ask-lumina.ts) — heeft `getProfile()` al
...
**Buiten scope fase 1:** [`src/lib/ai/analyze-entry.ts`](src/lib/ai/analyze-entry.ts) gebruikt een eigen prompt bij opslaan — personalisatie daar is optionele vervolgstap, niet nodig om fase 1 af te ronden.
...
**Wijzig** [`src/app/(app)/layout.tsx`](src/app/(app)/layout.tsx) naar async server component:
...
- [`src/middleware.ts`](../../src/middleware.ts) heeft nu geen profiel-query
- Onboarding-route [`src/app/(onboarding)/`](src/app/(onboarding)/) zit **buiten** `(app)` — geen redirect-loop
...
**Geen UI-wijzigingen nodig** in Instellingen — coachstijl blijft bewerkbaar via [`ProfileForm.tsx`](src/components/features/settings/ProfileForm.tsx). Hoofddoel/prioriteiten bewerken in instellingen is buiten scope fase 1.
...
| Wijzig | `src/lib/profile/complete-onboarding.ts` |
| Wijzig | `src/components/features/onboarding/OnboardingWizard.tsx` |
| Wijzig | `src/lib/ai/agent-prompt.ts` |
| Wijzig | `src/lib/ai/agent.ts` |
| Wijzig | `src/lib/ai/respond-to-entry.ts` |
| Wijzig | `src/lib/ai/ask-lumina.ts` |
| Wijzig | `src/app/(app)/layout.tsx` |
