...
- [`src/lib/ai/question-context.ts`](src/lib/ai/question-context.ts) — `vat_samen` type + lege question-categories
- [`src/lib/ai/toolbar-actions.ts`](src/lib/ai/toolbar-actions.ts) — aparte mapping + `resolveToolbarActionLabel`
- [`src/lib/ai/agent-prompt.ts`](src/lib/ai/agent-prompt.ts) — gesplitste prompts + thread-first user message voor `vat_samen`
- [`src/components/features/journal/JournalFlow.tsx`](src/components/features/journal/JournalFlow.tsx) — labelweergave via `resolveToolbarActionLabel`
