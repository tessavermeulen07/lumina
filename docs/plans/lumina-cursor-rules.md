...
todos:
  ...
  - id: lumina-project-rule
    ...
    status: completed
  - id: nextjs-rule
    content: Schrijf `api.mdc` voor src/app/** met Next.js 16 conventies
    status: completed
  - id: react-rule
  ...
  - id: optional-metadata
    content: Werk src/app/layout.tsx metadata bij naar Lumina (NL)
    status: completed
isProject: true
---
...
Lumina staat nog op de [create-next-app](https://nextjs.org) starter: Next.js **16.2**, React **19**, Tailwind **4**, TypeScript, App Router in [`src/app/`](src/app/). Cursor rules: `general.mdc`, `components.mdc`, `api.mdc`, `tailwind-styling.mdc`; zie ook [`AGENTS.md`](../AGENTS.md).
...
.cursor/rules/
  ...
  nextjs-app-router.mdc       # globs: src/app/**/*
...
- **Metadata:** [`src/app/layout.tsx`](src/app/layout.tsx) titel/description aanpassen naar Lumina (NL).
...
- Layouts: gedeelde shell in `src/app/(app)/layout.tsx`; pagina's blijven dun (compositie, geen grote UI-blokken inline).
- Route Handlers (later): `src/app/api/**/route.ts` met typed request/response.
...
**Globs:** `**/*.tsx` (eventueel beperken tot `src/components/**/*` + `src/app/**/*` als noise te veel is)

**Inhoud:**

- Functionele componenten + hooks; geen class components.
- Props via TypeScript interfaces (`Readonly<{...}>` waar passend, zoals in [`src/app/layout.tsx`](src/app/layout.tsx)).
- Splits presentatie (components) vs. pagina-compositie (app routes).
- Herbruikbare UI eerst in `src/components/ui/`; geen duplicate Button/Input per feature.
...
- Tailwind **v4** via [`src/app/globals.css`](src/app/globals.css) (`@import "tailwindcss"`, `@theme inline`) — geen legacy `tailwind.config` tenzij later toegevoegd.
...
|------|---------|--------------|
| `reflection-domain.mdc` | Journal/entries feature | `src/components/features/journal/**`, `src/components/features/journal/**` |
| `ai-integration.mdc` | AI-prompts/coaching | `src/lib/ai/**`, `src/app/api/**` |
| `data-privacy.mdc` | Auth + database | `src/lib/db/**`, server actions |
...
3. [`src/app/layout.tsx`](src/app/layout.tsx) metadata bijwerken naar Lumina (NL) — optioneel maar consistent met project-rule.
