...
- [ ] Maak `src/components/features/auth/LoginCard.tsx` met UI + Supabase signInWithPassword
- [ ] Maak `src/app/(marketing)/inloggen/page.tsx` met gecentreerde auth-shell
...
- Supabase clients in [`src/lib/supabase/client.ts`](src/lib/supabase/client.ts) en [`src/lib/supabase/server.ts`](src/lib/supabase/server.ts)
- Middleware in [`src/middleware.ts`](../../src/middleware.ts) stuurt niet-ingelogde gebruikers door (nu nog naar `/login`)
- [`src/components/features/marketing/RegisterCard.tsx`](src/components/features/marketing/RegisterCard.tsx) is het visuele referentiemodel (logo, kaart, formulier, footer-link)
- Dashboard = [`src/app/(app)/vandaag/page.tsx`](src/app/(app)/vandaag/page.tsx)
...
**Pagina-layout:** gecentreerde full-height shell (vergelijkbaar met [`src/app/(marketing)/layout.tsx`](src/app/(marketing)/layout.tsx) `marketing-aura`), zonder AppHeader. Footer verbergen op `/inloggen` via [`FooterGate`](src/components/layout/FooterGate.tsx) — net als onboarding, voor een rustige focus.

## Bestanden

### 1. `src/components/features/auth/LoginCard.tsx` (nieuw, client component)
...
### 2. `src/app/(marketing)/inloggen/page.tsx` (nieuw)
...
### 3. `src/components/ui/Input.tsx` — kleine uitbreiding
...
### 4. Middleware-aanpassingen — [`src/middleware.ts`](../../src/middleware.ts)
...
- [`src/components/features/marketing/Header.tsx`](src/components/features/marketing/Header.tsx): "Inloggen"-knop `href="/inloggen"`
- [`src/app/auth/callback/route.ts`](src/app/auth/callback/route.ts): redirect naar `/vandaag`

### 6. `src/app/wachtwoord-vergeten/page.tsx` (minimale stub)
