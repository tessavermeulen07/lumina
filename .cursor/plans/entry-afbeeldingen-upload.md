...
- De toolbar-knop opent al [`ImageUploadModal.tsx`](../../components/journal/ImageUploadModal.tsx), maar die is **mock** (geen file handling, geen upload).
...
Nieuw: [`lib/entries/upload-entry-image.ts`](../../lib/entries/upload-entry-image.ts) (`"use server"`)

Input: `entryId`, `FormData` met `file`

Stappen:

1. Auth via [`lib/supabase/server.ts`](../../lib/supabase/server.ts)
...
**Draft-entry zonder tekst:** als gebruiker alleen een afbeelding wil toevoegen vóór auto-save, roep in [`WritingArea.tsx`](../../components/journal/WritingArea.tsx) vóór upload `createEntryWithUserBlock('<p></p>')` aan (of kleine helper `ensureEntryDraft`) zodat `entryId` bestaat.

## 3. Afbeeldingen serveren (privé bucket)

Nieuw: [`app/api/entry-images/[...path]/route.ts`](../../app/api/entry-images/[...path]/route.ts)
...
- [`RichTextEditor.tsx`](../../components/journal/RichTextEditor.tsx): `Image.configure({ inline: false, allowBase64: false })`
- Prose-styling in [`rich-text-styles.ts`](../../lib/journal/rich-text-styles.ts): `[&_img]:my-4 [&_img]:max-w-full [&_img]:rounded-xl`

### EditorBridge

[`EditorBridge.tsx`](../../components/journal/EditorBridge.tsx): nieuwe methode `insertImage({ src, storagePath, alt? })`.
...
Uitbreiden [`lib/utils/rich-text.ts`](../../lib/utils/rich-text.ts):
...
Uitbreiden [`delete-entry.ts`](../../lib/entries/delete-entry.ts):
...
| Nieuw | `lib/entries/upload-entry-image.ts` |
| Nieuw | `lib/entries/ensure-entry-draft.ts` (optioneel) |
| Nieuw | `lib/utils/entry-images.ts` |
| Nieuw | `app/api/entry-images/[...path]/route.ts` |
| Wijzig | `components/journal/ImageUploadModal.tsx` |
| Wijzig | `components/journal/RichTextEditor.tsx` |
| Wijzig | `components/journal/EditorBridge.tsx` |
| Wijzig | `components/journal/WritingArea.tsx` |
| Wijzig | `lib/utils/rich-text.ts`, `lib/journal/rich-text-styles.ts` |
| Wijzig | `lib/entries/delete-entry.ts` |
