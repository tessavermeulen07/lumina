...
todos:
  - id: extract-logo
    content: LogoIcon extraheren naar components/ui/Logo.tsx en marketing Header updaten
    status: completed
  - id: app-header
    content: AppHeader, AppNav (actieve links) en UserMenu (dropdown) bouwen
    status: completed
  - id: app-layout-routes
    content: app/(app)/layout.tsx + routes /vandaag, /entries, /schrijf, /instellingen
    status: completed
  - id: mock-data
    content: lib/mock/dashboard.ts met weekdagen, stats en placeholder-prompts/vragen
    status: completed
  - id: dashboard-ui
...
De marketing-site staat onder [`app/(marketing)/`](app/(marketing)/). De ingelogde app komt in route group [`app/(app)/`](app/(app)/) met gedeelde layout.
...
Aparte cards: datum + weekcirkels links, stats rechts. Data via `getDashboardOverview()` → props. Types in `lib/types/dashboard.ts`.
...
- Goal-type in `lib/types/goal.ts`; lokale state (geen Supabase)
