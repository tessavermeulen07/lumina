...
**Probleem:** [`complete-onboarding.ts`](../../lib/profile/complete-onboarding.ts) slaat alleen `ai_persona_preference` op; `mainGoal`, `priorities` en `experience` verdwijnen in sessionStorage ([`OnboardingWizard.tsx`](../../components/onboarding/OnboardingWizard.tsx)).
...
- Update [`lib/types/database.ts`](../../lib/types/database.ts) `Profile`-interface.

**Opslag:**

- Breid [`complete-onboarding.ts`](../../lib/profile/complete-onboarding.ts) uit: ontvang volledige `OnboardingAnswers`, schrijf alle velden + `onboarding_completed_at`.
- Pas [`OnboardingWizard.tsx`](../../components/onboarding/OnboardingWizard.tsx) aan: geef `finalAnswers` door i.p.v. alleen `coachStyle`.

**AI-personalisatie:**

- Breid `buildSystemPrompt` in [`agent-prompt.ts`](../../lib/ai/agent-prompt.ts) uit met optionele `onboardingContext` (hoofddoel, prioriteiten, ervaring) — korte NL-contextblokken, geen lange prompts.
- Laad profielvelden in [`respond-to-entry.ts`](../../lib/ai/respond-to-entry.ts) en [`ask-lumina.ts`](../../lib/ai/ask-lumina.ts) en geef door aan `runLuminaAgent`.

**Onboarding afdwingen:**

- In [`app/(app)/layout.tsx`](../../app/(app)/layout.tsx): als ingelogde user `onboarding_completed_at` mist → redirect naar `/onboarding` (behalve `/onboarding` zelf en auth-routes).

---

## Fase 2 — Emotion Analytics: trends over tijd

**Probleem:** [`inzichten/page.tsx`](../../app/(app)/inzichten/page.tsx) roept `getWeeklyInsights()` aan zonder week-parameter; [`EmotionalLandscape.tsx`](../../components/insights/EmotionalLandscape.tsx) toont alleen een week-snapshot.

**Weeknavigatie (hergebruik patroon van geschiedenis):**

- Voeg `searchParams.week` toe aan [`inzichten/page.tsx`](../../app/(app)/inzichten/page.tsx).
- Geef `weekStartParam` door aan bestaande [`getWeeklyInsights`](../../lib/insights/get-weekly-insights.ts) (ondersteunt dit al).
- Nieuwe client-component `InsightsWeekHeader` — kopieer navigatielogica uit [`HistoryWeekHeader.tsx`](../../components/history/HistoryWeekHeader.tsx) / [`GeschiedenisView.tsx`](../../components/history/GeschiedenisView.tsx).

**Emotietrends (nieuw):**

- Nieuw `lib/insights/get-emotion-trends.ts`:
  ...
  - Hergebruik `startOfWeek` uit [`week-utils.ts`](../../lib/data/week-utils.ts).
- Nieuw `components/insights/EmotionTrendChart.tsx`:
  - Zelfde CSS-bar/lijn-aanpak als [`WordsPerDayChart.tsx`](../../components/insights/WordsPerDayChart.tsx) — **geen nieuwe chart-library** (project heeft geen recharts/d3).
...
**TwinWord-koppeling:** Geen extra API-werk nodig — scores komen al binnen via [`finalize-entry.ts`](../../lib/entries/finalize-entry.ts) → `emotion_analyses`.

---

## Fase 3 — Intenties & check-ins (in-app)

**Probleem:** [`GoalsSection.tsx`](../../components/dashboard/GoalsSection.tsx) is CRUD-only; `habit_logs` bestaat in schema maar wordt nergens gebruikt.

**Data-laag (nieuw in `lib/habits/`):**
...
- Nieuw `lib/ai/intention-checkin.ts`: korte `runLuminaAgent`-call met `interactionMode: "dashboard"`, intentie-context, optioneel `search_journal_history`.
- Sla gegenereerde vraag op in `habit_logs.ai_checkin_prompt` bij eerste weergave (cache).

**UI op `/vandaag`:**

- Nieuw `components/dashboard/IntentionCheckInCard.tsx` — toon alleen intenties die **due** zijn.
- Per kaart: AI-vraag, knoppen *Gedaan* / *Overgeslagen* / *Schrijf erover* (link naar `/schrijf` met query-param).
- Integreer in [`vandaag/page.tsx`](../../app/(app)/vandaag/page.tsx) boven of in [`GoalsAndLuminaRow.tsx`](../../components/dashboard/GoalsAndLuminaRow.tsx).

**Gewoontes vs intenties:** Voor scope: behoud UI-label "Intenties" maar ondersteun `type: 'habit'` in dezelfde modal (toggle of apart tabblad in [`AddGoalModal.tsx`](../../components/dashboard/AddGoalModal.tsx)). Database ondersteunt dit al.
...
- UI in [`ProfileForm.tsx`](../../components/settings/ProfileForm.tsx): opt-in toggle + tijdstip.

**E-mail versturen:**

- Voeg **Resend** toe (eenvoudig, gratis tier) of configureer Supabase SMTP — Resend is pragmatischer voor custom reminder-mails.
- Nieuw `lib/email/send-intention-reminder.ts`: NL-template ("Je intentie *X* wacht op je vandaag") + link naar `/vandaag`.

**Cron-job:**

- Nieuw `app/api/cron/intention-reminders/route.ts`:
  - Beveilig met `CRON_SECRET` header (Vercel Cron).
  - Service-role Supabase client ([`lib/supabase/admin.ts`](../../lib/supabase/admin.ts)).
...
- Pagina `/wachtwoord-vergeten` (route staat al in [`middleware.ts`](../../middleware.ts) public routes maar pagina ontbreekt).
...
- Nieuwe TwinWord-integratie — werkt al via [`twinword.ts`](../../lib/ai/twinword.ts).
