...
todos:
  - id: analysis-levels
    content: Nieuw lib/insights/analysis-levels.ts met drempels, labels en unlock-logica (zondag)
    status: pending
  - id: db-migration
    content: Migratie weekly_reports + types in database.ts en weekly-report.ts
    status: pending
  - id: ai-generate
    content: Nieuw lib/ai/generate-weekly-report.ts met niveau-afhankelijke prompt + ensure-weekly-report.ts
    status: pending
  - id: data-layer
...
Fase 2 leverde [`EmotionTrendChart`](../../components/insights/EmotionTrendChart.tsx) bovenaan [`inzichten/page.tsx`](../../app/(app)/inzichten/page.tsx). Gewenst is daar een **wekelijks rapport** — vergelijkbaar met de statistieken-sectie — met:
...
Weekgrenzen blijven **maandag–zondag** via bestaande [`startOfWeek`](../../lib/data/week-utils.ts). "Ontgrendelt op zondag" = rapport wordt beschikbaar op de zondag (einde week) of wanneer de gebruiker daarna `/inzichten` bezoekt.

---

## Stap 1 — Analyse-niveaus (woordendrempels)

Nieuw [`lib/insights/analysis-levels.ts`](../../lib/insights/analysis-levels.ts):
...
Woordentelling hergebruikt `totalWords` uit [`get-weekly-insights.ts`](../../lib/insights/get-weekly-insights.ts) (som `entry_analyses.word_count` per week).
...
Types in [`lib/types/database.ts`](../../lib/types/database.ts) + `WeeklyReportSection` in `lib/types/weekly-report.ts`.

**Niet** hergebruiken van `ai_insights` — die tabel is voor ad-hoc inzichten via Ask Lumina ([`save-insight.ts`](../../lib/ai/save-insight.ts)).

---

## Stap 3 — AI-generatie

Nieuw [`lib/ai/generate-weekly-report.ts`](../../lib/ai/generate-weekly-report.ts):
...
**Trigger:** [`lib/insights/ensure-weekly-report.ts`](../../lib/insights/ensure-weekly-report.ts) — aangeroepen vanuit data-laag wanneer:
...
Nieuw [`lib/insights/get-weekly-report.ts`](../../lib/insights/get-weekly-report.ts):
...
Aparte call in parallel met bestaande insights in [`inzichten/page.tsx`](../../app/(app)/inzichten/page.tsx) (scheiding van verantwoordelijkheden).

---

## Stap 5 — UI-componenten

Nieuwe sectie met heading [`insightsSectionHeadingClass`](../../components/insights/insights-styles.ts): **"wekelijks rapport"**.

### Huidige week — `WeeklyReportProgressCard`

Card in [`insightsCardClass`](../../components/insights/insights-styles.ts):
...
Modelleer op [`EntryDetailModal`](../../components/history/EntryDetailModal.tsx) (overlay, escape, focus):
...
Wijzig [`app/(app)/inzichten/page.tsx`](../../app/(app)/inzichten/page.tsx):
...
- [`EmotionTrendChart`](../../components/insights/EmotionTrendChart.tsx)
- [`get-emotion-trends.ts`](../../lib/insights/get-emotion-trends.ts)
- Import en call in page
- Optioneel: [`chart-axis.ts`](../../lib/insights/chart-axis.ts) als alleen door trend gebruikt — controleren en opruimen
...
| Actie | Bestand |
|-------|---------|
| Nieuw | `lib/insights/analysis-levels.ts` |
| Nieuw | `supabase/migrations/..._weekly_reports.sql` |
| Nieuw | `lib/types/weekly-report.ts` |
| Nieuw | `lib/ai/generate-weekly-report.ts` |
| Nieuw | `lib/insights/ensure-weekly-report.ts` |
| Nieuw | `lib/insights/get-weekly-report.ts` |
| Nieuw | `components/insights/WeeklyReportSection.tsx` |
| Nieuw | `components/insights/WeeklyReportProgressCard.tsx` |
| Nieuw | `components/insights/WeeklyReportSummaryCard.tsx` |
| Nieuw | `components/insights/WeeklyReportDetailModal.tsx` |
| Wijzig | `lib/types/database.ts` |
| Wijzig | `app/(app)/inzichten/page.tsx` |
| Verwijder | `components/insights/EmotionTrendChart.tsx` |
| Verwijder | `lib/insights/get-emotion-trends.ts` |
