...
- Bij page load `/vandaag`: als geen rijen met `prompt_date = vandaag` → roep [`generate-follow-up-prompts.ts`](../../lib/ai/generate-follow-up-prompts.ts) aan → INSERT 2–3 rijen
...
### Server actions — `lib/dashboard/reflection-prompt-actions.ts`
...
- Bookmark op schrijf-toolbar ([`WritingArea`](../../components/journal/WritingArea.tsx)) blijft **lokaal/UI-only** — los van dashboard-bookmarks
